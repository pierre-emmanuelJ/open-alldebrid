/*
Alldebrid API

Welcome to the OpenAPI Alldebrid API v4 !<br /> You can use this API to access various Alldebrid services from custom applications or scripts.<br /> <br /> API is organized around REST,<br /> returns JSON-encoded responses and use standard HTTP response codes.<br /> <br /> All calls are to be made on the HTTPS endpoints.<br /> Some are public, others require to be authentificated with an apikey (see Authentication).<br /> <br /> You must also identify your apps or script with a meaningfull agent parameter.<br /> <br /> This API version is namespaced as v4, as such all endpoint start with /v4/,<br /> such like http://api.alldebrid.com/v4/ping?agent=apiShowcase.<br /> <br /> This API v4 should be the final version regarding general response format and errors (hopefully).<br />

API version: 4.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// InlineResponse20011Data struct for InlineResponse20011Data
type InlineResponse20011Data struct {
	Files *[]InlineResponse20011DataFiles `json:"files,omitempty"`
}

// NewInlineResponse20011Data instantiates a new InlineResponse20011Data object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse20011Data() *InlineResponse20011Data {
	this := InlineResponse20011Data{}
	return &this
}

// NewInlineResponse20011DataWithDefaults instantiates a new InlineResponse20011Data object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse20011DataWithDefaults() *InlineResponse20011Data {
	this := InlineResponse20011Data{}
	return &this
}

// GetFiles returns the Files field value if set, zero value otherwise.
func (o *InlineResponse20011Data) GetFiles() []InlineResponse20011DataFiles {
	if o == nil || o.Files == nil {
		var ret []InlineResponse20011DataFiles
		return ret
	}
	return *o.Files
}

// GetFilesOk returns a tuple with the Files field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20011Data) GetFilesOk() (*[]InlineResponse20011DataFiles, bool) {
	if o == nil || o.Files == nil {
		return nil, false
	}
	return o.Files, true
}

// HasFiles returns a boolean if a field has been set.
func (o *InlineResponse20011Data) HasFiles() bool {
	if o != nil && o.Files != nil {
		return true
	}

	return false
}

// SetFiles gets a reference to the given []InlineResponse20011DataFiles and assigns it to the Files field.
func (o *InlineResponse20011Data) SetFiles(v []InlineResponse20011DataFiles) {
	o.Files = &v
}

func (o InlineResponse20011Data) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Files != nil {
		toSerialize["files"] = o.Files
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse20011Data struct {
	value *InlineResponse20011Data
	isSet bool
}

func (v NullableInlineResponse20011Data) Get() *InlineResponse20011Data {
	return v.value
}

func (v *NullableInlineResponse20011Data) Set(val *InlineResponse20011Data) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse20011Data) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse20011Data) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse20011Data(val *InlineResponse20011Data) *NullableInlineResponse20011Data {
	return &NullableInlineResponse20011Data{value: val, isSet: true}
}

func (v NullableInlineResponse20011Data) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse20011Data) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


